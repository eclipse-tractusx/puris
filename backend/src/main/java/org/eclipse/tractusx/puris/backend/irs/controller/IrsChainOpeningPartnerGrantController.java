...
/**
 * Exposes the IRS partner grants to the frontend. Read-only
 * The class level {@code @PreAuthorize} applies so all endpoints are admin-only.
 */
@RestController
@RequestMapping("irs/partner-grants")
@Slf4j
public class IrsChainOpeningPartnerGrantController {
    @Autowired
    ...
    @GetMapping()
    @ResponseBody
    @Operation(summary = "Get all chain opening partner grants -- ADMIN ONLY",
        description = "Get all partner grants regardless of their sync status " +
            "(NOT_SYNCED, PENDING, SYNCED, OUT_OF_SYNC, DELETED). Only available if the IRS adapter is enabled.")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Chain opening partner grants were returned."),
    ...
    public List<IrsChainOpeningGrantDto> getAllPartnerGrants() {
        if (!irsRequestService.isEnabled()) {
            log.warn("Rejected request for IRS chain opening partner grants because the IRS adapter is disabled.");
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "IRS adapter is disabled. Enable it via puris.irsadapter.enabled to access IRS chain opening partner grants.");
        }
        return irsChainOpeningPartnerGrantService.findAll().stream().map(this::convertToDto).toList();
    }
 
    private IrsChainOpeningGrantDto convertToDto(IrsChainOpeningPartnerGrant entity) {

...
Consider these quick guides for specific topics.

### Authentication

Two sections are relevant:

- postgresql.auth - define a superuser with custom name
- postgresql.customUser - define a custom non-root user (optional but **recommended**)

All templates allow to overwrite values, if the respective key is overwritten

```yaml
postgresql:
  auth:
    existingSecret: "admin-secret-name" # will be used / created and overriden
    database: "postgres" # will override
    username: "postgres" # will override
    password: "your-admin-password-eg-from-vault" # will override
  customUser:
    existingSecret: "custom-secret-name" # will be used / created and overriden
    database: "puris" # will override
    name: "puris" # will override
    password: "your-customer-user-password-eg-from-vault" # will override
```

To support the previous mechanism, a catch all secret may be created if `postgresql.auth.existingSecret` is set.It also contains the customUser information, if customUser has name or existingSecret (also same name is considered).

### Autocreation of Separate Secrets (**RECOMMENDED**)
...
Results in two secret:

- `{Release Name}-postgresql` updated with `postgres-password`.
- `{Release Name}-postgresql-custom-user-credentials` updated with `CUSTOM_PASSWORD`, `CUSTOM_USER`, `CUSTOM_DB`.

### Reusage and Update of Secrets

Beside the following options, for existingSecrets, also specific secretKeys may be used.

For `postgresql.auth` the following keys may be specified:

- `adminPasswordKey`: default `postgres-password`

For `postgresql.customUser` the following keys may be specified:

- `name`: default `CUSTOM_USER`
- `password`: default `CUSTOM_PASSWORD`
- `database`: default `CUSTOM_DB`

> [!note]
> If postgres is disabled (`postgresql.enabled: false`), no secret for postgres is created, then the secret for the backend is created based on the `backend.puris.datasource.password` value.

#### Option A: Existing admin secret with automatically created custom user secret
...
- `existing` secret updated with `postgres-password`.
- `{Release Name}-postgresql-custom-user-credentials` secret updated with `CUSTOM_PASSWORD`, `CUSTOM_USER`, `CUSTOM_DB`.

#### Option B: Existing custom user secret with automatically created admin secret
...
- `{Release Name}-postgresql` updated with `postgres-password`, `host`, `port`, `username`, `database`, `uri`.
- `existing` secret updated with `CUSTOM_PASSWORD`, `CUSTOM_USER`, `CUSTOM_DB`.

#### Option C: Shared Existing Secret

> [!note]
> This represents the old way and is **NOT RECOMMENDED**.
...
Results in one secret (`existing`) that will be updated with the `postgres-password`, `CUSTOM_PASSWORD`, `CUSTOM_USER`, `CUSTOM_DB`.
...
| backend.puris.batch.partnerdataupdate.cleanup.cron | string | `"0 0 4 * * *"` | cron expression for cleanup (default: daily at 04:00) |
| backend.puris.batch.partnerdataupdate.cleanup.enabled | bool | `true` | toggle cleanup scheduler |
| backend.puris.batch.partnerdataupdate.cleanup.retentiondays | int | `30` | number of days after which the old batch runs are removed |
| backend.puris.batch.partnerdataupdate.cron | string | `"0 0 9 * * *"` | cron for spring @Scheduled batch, default daily at 9.00 |
| backend.puris.batch.partnerdataupdate.enabled | bool | `true` | if true, the partner data update is triggered for all materials and partners onboarded; else not. |
| backend.puris.dataexchangerequestapi.apiassetid | string | `"dataexchangerequestapi-api-asset"` | Asset ID for Data Exchange Request API |
...
| backend.puris.deliveryanonymizedsubmodel.apiassetid | string | `"deliveryanonymizedsubmodel-api-asset"` | Asset ID for DeliveryAnonymizedSubmodel API |
| backend.puris.deliverysubmodel.apiassetid | string | `"deliverysubmodel-api-asset"` | Asset ID for DeliverySubmodel API |
| backend.puris.demandanonymizedsubmodel.apiassetid | string | `"demandanonymizedsubmodel-api-asset"` | Asset ID for DemandAnonymizedSubmodel API |
...
| backend.puris.irsadapter | object | `{"adminauthsecret":"my-irs-admin-secret","authkey":"x-api-key","authsecret":"","enabled":false,"job":{"polldelayseconds":300},"queue":{"backoffmultiplier":2,"initialretrydelayseconds":30,"maxattempts":5,"maxretrydelayseconds":3600,"pollintervalseconds":30},"url":"https://my-irs-url:8080"}` | Configuration for the IRS adapter. The IRS adapter is used to send requests to the IRS for data exchange. The IRS is not part of the PURIS application and must be provided by the user. Please note that in addition to enabled = true, the IRS url must be specified and the adminauthsecret must be set correctly for the IRS adapter to be active. |
| backend.puris.irsadapter.adminauthsecret | string | `"my-irs-admin-secret"` | The auth secret to be used on your IRS adapter's request api. The secret requires admin permissions. Reused from existing secret. Secret key "puris-irsadapter-adminauthsecret". |
| backend.puris.irsadapter.authkey | string | `"x-api-key"` | Header key used for IRS API authentication. |
| backend.puris.irsadapter.authsecret | string | `""` | The auth secret to be used on your IRS adapter's request api for non-admin requests. Optional - if omitted, the adminauthsecret is used instead. Reused from existing secret. Secret key "puris-irsadapter-authsecret". |
| backend.puris.irsadapter.enabled | bool | `false` | Toggles usage of the IRS adapter |
| backend.puris.irsadapter.job.polldelayseconds | int | `300` | Delay (in seconds) before polling an IRS job's status, both for the first poll after job creation succeeds and for every subsequent poll while the job is still in progress. |
| backend.puris.irsadapter.queue.backoffmultiplier | int | `2` | Multiplier applied to the retry delay after each failed attempt. |
| backend.puris.irsadapter.queue.initialretrydelayseconds | int | `30` | Delay (in seconds) before the first retry of a failed queued IRS request. |
| backend.puris.irsadapter.queue.maxattempts | int | `5` | Maximum number of attempts (including the first) made for a queued IRS request before it is marked permanently failed. |
| backend.puris.irsadapter.queue.maxretrydelayseconds | int | `3600` | Upper bound (in seconds) on the delay between two attempts of a queued IRS request. |
| backend.puris.irsadapter.queue.pollintervalseconds | int | `30` | How often (in seconds) the IRS request queue worker checks for due requests. |
| backend.puris.irsadapter.url | string | `"https://my-irs-url:8080"` | Base URL of the external IRS service. Must contain protocol (http/https). |
| backend.puris.itemstockanonymizedsubmodel.apiassetid | string | `"itemstockanonymizedsubmodel-api-asset"` | Asset ID for ItemStockAnonymizedSubmodel API |
...
| backend.puris.policyprofile.version | string | `"profile2509"` | The default policy profile to use for policy definitions and contract definitions. If you intend to use an EDC before Tractus-x EDC version 0.11.0 or equivalent this needs to be "profile2405". For newer EDC versions (>=0.11.0) you can choose to use the newest version profile2509 or the legacy version profile2405. |
| backend.puris.productionanonymizedsubmodel.apiassetid | string | `"productionanonymizedsubmodel-api-asset"` | Asset ID for ProductionAnonymizedSubmodel API |
...
| backend.puris.singlelevelbomasplannedsubmodel.apiassetid | string | `"singlelevelbomasplannedsubmodel-api-asset"` | Asset ID for SingleLevelBomAsPlannedSubmodel API |
...
| frontend.puris.endpointDataExchangeApproval | string | `"data-exchange-approval"` | The endpoint for data exchange approvals |
| frontend.puris.endpointDataExchangeRequest | string | `"data-exchange-request"` | The endpoint for data exchange requests |
...
| frontend.puris.endpointPartnerDataUpdateBatch | string | `"batch/partner-data-update"` | The endpoint for interfacing with partner data update batch |
...
| postgresql.auth.database | string | `"postgres"` | Database name of the root user. Must be different than customUser if provided. |
...
| postgresql.auth.password | string | `""` | Password of the root user |
| postgresql.auth.username | string | `"postgres"` | Username of the root user |
| postgresql.customUser.database | string | `"puris"` | Name for the custom database to be created and assigned to the custom user |
| postgresql.customUser.name | string | `"puris"` | Name of the custom user to be created |
| postgresql.customUser.password | string | `""` | Password to be used for the custom user |

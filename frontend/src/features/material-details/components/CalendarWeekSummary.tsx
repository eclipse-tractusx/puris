        ...
        <Stack flex={isExpanded ? 50 : 10} sx={{ borderRight: '1px solid #e5e5e5', minWidth: isExpanded ? '34rem' : '9rem' }}>
            {showHeader && (
                <Stack
                    ...
                    alignItems="center"
                >
                    <Typography variant="body2" component="h4" color={theme.palette.primary.contrastText}>{`CW ${cw.week.toString().padStart(2, '0')}`}</Typography>
                    <IconButton
                        sx={{ position: 'absolute', top: '0.125rem', right: '0.125rem', color: theme.palette.primary.contrastText, fontSize: '1rem', }}
                        onClick={() => onToggleExpanded?.(!isExpanded)}
                    > {isExpanded ? <CloseFullscreenOutlined /> : <OpenInFullOutlined />} </IconButton>
                </Stack>
            )}
            <Grid container columns={8} flex={1} height="100%">
                            ...
                            <Grid key={date.toLocaleDateString()} item xs={1} height="100%">
                                <Stack height="100%">
                                    ...
                                    <Box flex={1} display="flex" justifyContent="center" alignItems="center">
                                        ...
                                        >
                                            <Typography variant="body2"> {summary.dailySummaries[date.toLocaleDateString()]?.primaryValueTotal} </Typography>
                                        </Button>
                                    </Box>
                                    <Box flex={1} display="flex" justifyContent="center" alignItems="center">
                                        <Button
                                        ...
                                        >
                                            <Typography variant="body2"> {summary.dailySummaries[date.toLocaleDateString()]?.deliveryTotal} </Typography>
                                        </Button>
                                    </Box>
                                    <Box flex={1} display="flex" justifyContent="center" alignItems="center">
                ...
                <Grid item xs={isExpanded ? 1 : 8} height="100%">
                    <Stack height="100%" sx={{ borderLeft: isExpanded ? '1px solid #e5e5e5' : 'none', backgroundColor: isExpanded ? 'white' : '#f5f5f5' }}>
                        {showHeader && (
                            <Box
                                height="2.5rem"
                                display="flex"
                            ...
                            >
                                <Stack alignItems="center" spacing={0.25}>
                                    ...
                                    {!isExpanded && (
                                        <Typography variant="body3" color="#777">{`${cw.startDate.toLocaleDateString()} - ${incrementDate(cw.startDate, 6).toLocaleDateString()}`}</Typography>
                                    )}
                                </Stack>
                            </Box>
                        )}
                        <Box flex={1} display="flex" justifyContent="center" alignItems="center">
                            <Typography variant="body2">
                                {weekDates.reduce((total, date) => total + (summary.dailySummaries[date.toLocaleDateString()]?.primaryValueTotal ?? 0), 0)}
                            </Typography>
                        </Box>
                        <Box flex={1} display="flex" justifyContent="center" alignItems="center">
                            <Typography variant="body2">
                                {weekDates.reduce((total, date) => total + (summary.dailySummaries[date.toLocaleDateString()]?.deliveryTotal ?? 0), 0)}
                            </Typography>
                        </Box>
                        <Box flex={1} display="flex" justifyContent="center" alignItems="center">
                            <Typography

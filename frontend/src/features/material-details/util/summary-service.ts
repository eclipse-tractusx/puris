...
type SummaryTypeMap = { production: Production; demand: Demand; };
...
export type Summary<TType extends SummaryType> = { type: TType; dailySummaries: Record<string, DailySummary<TType>>; };
...
function getDateValue<TType extends SummaryType>(type: TType, entity: SummaryTypeMap[TType]): Date {
    return new Date(type === 'production'
...
export function createSummary<TType extends SummaryType>(
    ...
    const groupedDeliveries = groupBy(deliveries, (item) => new Date(type === 'production' ? item.dateOfDeparture : item.dateOfArrival).toLocaleDateString());
    const summary: Summary<TType> = { type: type, dailySummaries: {}, };
    ...
    for (let i = 0; i <= timespan; i++) {
        ...
        const dailySummary: DailySummary<TType> = {
            ...
            stocks: i === 0 ? stocks ?? [] : [],
            stockTotal: i === 0 ? stocks.reduce((sum, s) => sum + s.quantity, 0) : summary.dailySummaries[previousDateString].stockTotal + stockDelta,
        };

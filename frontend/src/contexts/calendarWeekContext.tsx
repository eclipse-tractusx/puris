...
type CalendarWeekContext = { calendarWeeks: Expandable<CalendarWeek>[]; expandWeek: (state: boolean, index: number) => void; };

const calendarWeekContext = createContext<CalendarWeekContext | null>(null);

type CalendarWeekProviderProps = { children: ReactNode; };
...
export const CalendarWeekProvider = ({ children }: CalendarWeekProviderProps) => {
    ...
    const expandWeek = (state: boolean, index: number) => setCalendarWeeks((prev) => prev.map((cw, i) => (i === index ? { ...cw, isExpanded: state } : cw)));
    return <calendarWeekContext.Provider value={{ calendarWeeks, expandWeek }}>{children}</calendarWeekContext.Provider>;
};

export function useCalendarWeeks() {
    const context = useContext(calendarWeekContext);
    if (context === null) throw new Error('useCalendarWeeks must be used within a CalendarWeekProvider');
    return context;
}
